...
func Init(fileName string) {
	mode := zap.LevelEnablerFunc(func(lvl zapcore.Level) bool {
		switch mviper.GetString("plugin.logger.system_log.mode") {
		case "debug":
	...
	hook := &lumberjack.Logger{
		Filename:   filepath.Join(mviper.GetString("plugin.logger.system_log.path"), fileName),
		MaxSize:    mviper.GetInt("plugin.logger.system_log.max_size"),    // MB
		MaxBackups: mviper.GetInt("plugin.logger.system_log.max_backups"), // how many backup for log file.
		MaxAge:     mviper.GetInt("plugin.logger.system_log.max_age"),
		Compress:   mviper.GetBool("plugin.logger.system_log.compress"),
	}
	...
	if mviper.GetBool("plugin.logger.system_log.show_in_console") {
